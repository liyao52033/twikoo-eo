...
async function capRedeem(event) {
  ...
  await storage.challenges.delete(token)
  // solutions 是与 challenge 顺序一一对应的扁平 nonce 数组：solutions[i] 对应 challenge[i]
  const results = await Promise.all(challengeData.challenge.map(([salt, target], i) => {
    const nonce = solutions[i]
    if (nonce === undefined || nonce === null) return false
    return sha256Hex(salt + nonce).then(hash => hash.startsWith(target))
  }))
